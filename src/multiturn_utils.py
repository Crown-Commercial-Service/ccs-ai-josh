...
def _build_synthesis_prompt(sql_context_str: str, safe_context: str) -> str:
    ...
    if _has_valid_sql_results(sql_context_str):
        sql_rules = (
            ...
            "2. NO DENIAL HALLUCINATIONS: If structured SQL data is present in <sql_database_context>, NEVER state or claim "
            "that the SQL database does not provide the requested breakdown (e.g., top customers for a supplier). Rely strictly "
            "on the returned SQL rows to answer the question directly.\n"
            "3. OPENING ANCHOR (MANDATORY): Always open Sentence 1 using the official SQL figures as the authoritative "
            "data (for example, 'The top 5 customers by spend for Accenture are...'). State the requested entity and time period "
            "whenever available in the question, conversation, SQL query, or SQL result.\n"
            "4. DOCUMENT SUB-TOTALS & MARKET REPORTS: PDF market reports and unstructured documents provide partial or "
            "qualitative context only. Never claim market report figures override, replace, or contradict the primary SQL totals. "
            "Report genuine scope differences clearly without casting doubt on the authoritative filtered SQL total.\n"
            "5. SQL-ONLY ANSWERS: Use SQL numbers to answer directly even if no documents were retrieved. Never emit a "
            "no-question or request-for-information placeholder when SQL data is present.\n"
            "6. SQL FILTER TRUST: Structured SQL data in <sql_database_context> is the absolute source of truth. If SQL filtered "
            "on a customer or supplier, its output is the official result for that organization. Never describe a filtered result "
            "as unfiltered, not specific to the entity, or broader than requested.\n"
