...
### Document URLs
To link to the documents that an answer is based on, we use a file called `CI_document_URLs.csv`, which is a table of file names and URLs. This is generated by running the script `/scripts/extractCIDocsURLs.appscript` from within Google Workspace. For full instructions on how to create this table, see the internal documentation.
