...
results["Document Match"] = pd.Categorical(
    results["Document Match"].astype(str), categories=["True", "False"], ordered=True
)
...
results_pos = (
    results[results["Question Type"] == "Positive"].copy().reset_index(drop=True)
)

pos_results_by_doc_type = (
    results_pos.groupby("Document Type")[["Correctness", "Retrieval", "Groundedness"]]
    .mean()
    .round(2)
)
print("Positive control results by doc type:")
print(pos_results_by_doc_type)
pos_results_by_doc_type.to_csv(
    os.path.join("data", "pos_results_by_doc_type.csv"), index=True
)
...
if not results_pos[results_pos["Document Type"] == "Doc Search"].empty:
    plt.subplot(1, 3, 1)
    sns.histplot(
        data=results_pos[results_pos["Document Type"] == "Doc Search"],
        x="Correctness",
        stat="percent",
    )
...
if not results_pos[results_pos["Document Type"] == "Fact Sheet"].empty:
    plt.subplot(1, 3, 2)
    sns.histplot(
        data=results_pos[results_pos["Document Type"] == "Fact Sheet"],
        x="Correctness",
        stat="percent",
    )
...
if not results_pos[results_pos["Document Type"] == "Market Report"].empty:
    plt.subplot(1, 3, 3)
    sns.histplot(
        data=results_pos[results_pos["Document Type"] == "Market Report"],
        x="Correctness",
        stat="percent",
    )
...
results_neg = (
    results[results["Question Type"] == "Negative"].copy().reset_index(drop=True)
)

neg_results_by_doc_type = (
    results_neg.groupby("Document Type")[["Correctness", "Retrieval", "Groundedness"]]
    .mean()
    .round(2)
)
print("Negative control results by doc type:")
print(neg_results_by_doc_type)
neg_results_by_doc_type.to_csv(
    os.path.join("data", "neg_results_by_doc_type.csv"), index=True
)
...
if not results_neg[results_neg["Document Type"] == "Fact Sheet"].empty:
    plt.subplot(1, 2, 1)
    sns.histplot(
        data=results_neg[results_neg["Document Type"] == "Fact Sheet"],
        x="Correctness",
        stat="percent",
    )
    plt.title("Fact Sheet")
if not results_neg[results_neg["Document Type"] == "Market Report"].empty:
    plt.subplot(1, 2, 2)
    sns.histplot(
        data=results_neg[results_neg["Document Type"] == "Market Report"],
        x="Correctness",
        stat="percent",
    )
    plt.title("Market Report")
